...
class GeoDataset(Dataset):
    def __init__(self, image_folder, annotation_file, tag='image_id'):
        ...
        self.gt = [(g[1][tag], g[1]['latitude'], g[1]['longitude']) for g in gt.iterrows()]
        self.tag = tag
...
if __name__ == '__main__':
    ...
    parser.add_argument('--annotation_file', type=str, required=False, default='train.csv')
    parser.add_argument('--features_parent', type=str, default='/home/isig/gaia-v2/faiss/street-clip')
    parser.add_argument('--data_parent', type=str, default='/home/isig/gaia-v2/loic-data/')
    ...
    if not os.path.isfile('text_street-clip-features.pkl'):
        if not os.path.isfile('rg_cities1000.csv'):
            os.system('wget https://raw.githubusercontent.com/thampiman/reverse-geocoder/master/reverse_geocoder/rg_cities1000.csv')

        cities = pd.read_csv('rg_cities1000.csv')
        cities = cities[['lat', 'lon', 'name', 'admin1', 'admin2', 'cc']]
        reprs = {0: {}, 1: {}, 2: {}, 3: {}}
        for line in tqdm(cities.iterrows(), total=len(cities), desc='Creating hierarchy'):
            lat, lon, city, region, sub_region, cc = line[1]
            try:
                city, region, sub_region, cc = [('' if pd.isna(x) else x) for x in [city, region, sub_region, coco.convert(cc, to='name_short')]]
                a, b, c, d = get_prompts(cc, region, sub_region, city)
                if a is not None:
                    if a not in reprs[0]:
                        reprs[0][a] = {'gps': {(lat, lon)}, 'embedding': compute_text_features_clip(a)}
                    else:
                        reprs[0][a]['gps'].add((lat, lon))

                if b is not None:
                    if b not in reprs[1]:
                        reprs[1][b] = {'gps': {(lat, lon)}, 'embedding': compute_text_features_clip(b)}
                    else:
                        reprs[1][b]['gps'].add((lat, lon))

                if c is not None:
                    if c not in reprs[2]:
                        reprs[2][c] = {'gps': {(lat, lon)}, 'embedding': compute_text_features_clip(c)}
                    else:
                        reprs[2][c]['gps'].add((lat, lon))

                if d is not None:
                    if d not in reprs[3]:
                        reprs[3][d] = {'gps': {(lat, lon)}, 'embedding': compute_text_features_clip(d.replace(', , ', ', '))}
                    else:
        ...
        for i in range(4):
            for k in reprs[i].keys():
                reprs[i][k]['gps'] = tuple(np.array(list(reprs[i][k]['gps'])).mean(axis=0).tolist())
    ...
    def matches(embed, repr, control, gt, sw=None):
        first_max = max(((k, embed.dot(v['embedding'])) for k, v in repr.items() if sw is None or k.startswith(sw)), key=operator.itemgetter(1))
            ...
            if flag:
                ...
                if xb is not None:
                    ...
                    if flag:
                        ...
                        if xc is not None:
                            N['sub-region'] += 1
                            gps, flag = matches(embed, reprs[2], reprs[1][xb], xc, sw=xb)
                            if flag:
                                pos['sub-region'] += 1
                                if xd is not None:
                                    N['city'] += 1
                                    gps, flag = matches(embed, reprs[3], reprs[2][xc], xd, sw=xc)
                                    if flag:
                                        pos['city'] += 1
                        else:
                            if xd is not None:
                                N['city'] += 1
                                gps, flag = matches(embed, reprs[3], reprs[1][xb], xd, sw=xb + ', ')
                                if flag:
                                    pos['city'] += 1
    ...
    test_gt = pd.read_csv(test_path_csv, dtype={'id': str})[['id', 'latitude', 'longitude']]
    test_gt = {g[1]['id']: np.array([g[1]['latitude'], g[1]['longitude']]) for g in tqdm(test_gt.iterrows(), total=len(test_gt), desc='Loading test_gt')}
