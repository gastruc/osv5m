...
def compute_features(path, data_dir, csv_file, tag, args):
    ...
    if not os.path.isdir(test_features_dir) or len(os.listdir(test_features_dir)) != len(data):
        device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
        model, transform, inference, collate_fn = get_backbone(args.name)
        dataloader = DataLoader(data, batch_size=args.batch_size, shuffle=False, num_workers=8, collate_fn=collate_fn)
...
def get_results(args, train_test)
    ...
    if not os.path.isfile(join(args.features_parent, '.cache', '1-nn.pkl')):
        ...
        indexes = [get_filenames(idx) for idx in tqdm(range(1, 6), desc='Loading indexes...')]

        train_gt = pd.read_csv(join(args.data_parent, args.annotation_file), dtype={'image_id': str})[['image_id', 'latitude', 'longitude']]
        test_gt = pd.read_csv(test_path_csv, dtype={'id': str})[['id', 'latitude', 'longitude']]

        # make a map between image_id and lat/lon
        train_gt = {g[1]['image_id']: np.array([g[1]['latitude'], g[1]['longitude']]) for g in tqdm(train_gt.iterrows(), total=len(train_gt), desc='Loading train_gt')}
        ...
        for f in tqdm(os.listdir(test_features_dir)):
            ...
            for index, ids in indexes:
                ...
                bisect.insort(neighbors, (ids[indices], distances), key=operator.itemgetter(1))
